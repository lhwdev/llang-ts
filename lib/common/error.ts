...
export function detailedError<E extends Error>(
  error: new (message?: string, options?: ErrorOptions) => E,
  options?: ErrorOptions,
): (strings: TemplateStringsArray, ...args: any[]) => E;

export function detailedError(strings: TemplateStringsArray, ...args: any[]): Error;

export function detailedError(a: any, ...args: any[]): any {
  if (typeof a === "function") {
    const options = args[0];
    return (strings: TemplateStringsArray, ...args: any[]) =>
      detailedErrorInternal(fmt(strings, ...args).s, a, options);
  }
  return detailedErrorInternal(fmt(a, ...args).s, Error);
}
function detailedErrorInternal<E extends Error>(
  str: string,
  error: new (message?: string, options?: ErrorOptions) => E,
  options?: ErrorOptions,
): E {
